...
function showUpdateForm(
  ...
  (document.getElementById("updateItemName") as HTMLInputElement).value =
    itemName;
  (document.getElementById("updateItemPrice") as HTMLInputElement).value =
    itemPrice.toString();
  (document.getElementById("addItemForm") as HTMLElement).style.display =
    "none";
}
...
function deleteItem(itemId: string): void {
  if (confirm("Are you sure you want to delete this item?")) {
    fetch(
      new Request(server + `api/crud/delete/${itemId}`, {
        method: "POST",
      }),
    )
      .then(() => reload());
